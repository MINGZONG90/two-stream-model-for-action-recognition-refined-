...
def train_1epoch(epoch):
    ...
    for i, sample in enumerate(train_loader):
        # get the inputs # data_dict = {'img2':[torch.FloatTensor of size 10x3x224x224],'img1':10x3x224x224,'img0':10x3x224x224]}
        data_dict, label = sample
        ...
        for j in range(len(data_dict)):
            ...
            # forward
            output += model(input) #Variable containing:[torch.cuda.FloatTensor of size 10x101 (GPU 0)]
...
for epoch in range(nb_epochs):    # loop over the dataset multiple times
    ...
    if is_best:
        best_prec1 = prec1
        with open('record/spatial/spatial_video_preds_best.pickle', 'wb') as f:
    ...
    }, is_best, 'record/spatial/checkpoint.pth', 'record/spatial/model_best.pth')

with open('record/spatial/spatial_video_preds_collections.pickle','wb') as f:
    pickle.dump(video_pred_collections, f)
f.close()

model_best = 'record/spatial/model_best.pth'
