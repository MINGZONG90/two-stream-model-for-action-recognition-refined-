...
class motion_dataset(Dataset):  
    ...
    def stackopf(self):                              #spatial does not have
        ...
        i = int(self.frames_idx)                 #107    #10 or 7 or ...
    ...
    def __getitem__(self, idx):          #spatial different
        ...
        if self.mode == 'train':
            self.video, nb_frames = self.keys[idx].split('-')
        #    self.frames_idx = random.randint(1,int(nb_frames))
            self.frames_idx = (1 + int(nb_frames)) / 4          # Note

        elif self.mode == 'val':
            self.video,self.frames_idx = self.keys[idx].split('-')
...
class Motion_DataLoader():
    def __init__(self, BATCH_SIZE, num_workers, in_channel,  path, UCF_list):      #spatial does not have
    ...
    def val_sample19(self):       #spatial different
        ...
        for video in self.test_video:
            ...
            for index in range(19):
                frame_idx = index*sampling_interval           #0
                key = video + '-' + str(frame_idx+1)          #PommelHorse_g01_c03-1
